...
public sealed class MessageService : IMessageService
{
    ...
    public async Task<Result<int>> CreateAsync(int senderUserId, CreationMessageRequest creationMessageRequest)
    {
        ...
        {
            var receiverId = creationMessageRequest.ReceiverId.Value;
            
            var receiverUserResult = await _userService.IsUserExistAsync(receiverId);
            
            if (receiverUserResult is false)
                return Result.Failure<int>(DomainErrors.User.NotFound(receiverId));
        }
        
        // Check if file exist
        if (creationMessageRequest.File is not null)
        {
            var fileResult = await _messageFileService.GetById(creationMessageRequest.File.MessageId);
