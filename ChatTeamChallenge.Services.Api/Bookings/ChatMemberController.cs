...
public sealed class ChatMemberController : ApiController
{
    ...
    {
        var result = await _chatMemberService.GetAll(page, pageSize, chatId, userId);
        return this.FromResult(result);
    }
    
    [HttpDelete(ApiRoutes.ChatMember.Remove)]
    public async Task<IActionResult> RemoveUser([FromBody] ChatMemberRequest chatMemberRequest)
    {
        if (!ModelState.IsValid)
        {
            return BadRequest(ModelState);
